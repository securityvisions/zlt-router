# Rollback snapshot 20260822-0300 — restore procedure

Ticket 01 of x28-smart-reports. Taken while the X28 was fully
...
|---|---|---|
| Device tarball (root-only, 600) | `/data/proxy/backup/rollback-20260822-0300.tar.gz` (~20 KB) | Everything below **plus the real xray configs and the v2raya DB** (secrets) |
| Repo copy (secret-free) | `router/x28/backup/rollback-20260822-0300/` | All scripts, init files, generated-config + iptables/ps/opkg state dumps, and **redacted** xray configs (`tmp/rollback-redacted/`), with `MANIFEST.sha256` |
...
tar xzf /data/proxy/backup/rollback-20260822-0300.tar.gz -C /
...
## Drill evidence (read-only, performed 2026-08-22 03:00)

- Extracted the device tarball to a scratch dir and `cmp`-verified every
  file against its live original: **identical, 0 mismatches**
