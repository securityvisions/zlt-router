...
# ---- calendar + time + rendering (re-exported from cal-lib.sh) ----
_cal_dir="$(cd "$(dirname "${0:-$0}")" 2>/dev/null && pwd)"
[ -f "$_cal_dir/cal-lib.sh" ] || _cal_dir="${HN_LIB%/*}"
[ -f "$_cal_dir/cal-lib.sh" ] || _cal_dir="/data/proxy"
CAL_LIB="$_cal_dir/cal-lib.sh"
